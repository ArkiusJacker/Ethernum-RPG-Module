...
import {
  getCatalogProfileOptions,
  getUniqueMechanicProfileMetadata,
  isCatalogProfileId,
} from '../../mechanics/profile-catalog.js';
import {
  DEFAULT_GYRO_STATE,
  GYRO_PROFILE_ID,
...
import {
  ...
  serializeExecutionMutation,
  transitionUniqueExecution,
  upsertUniqueExecution,
} from '../core/UniqueExecutionManager.js';
...
function getProfileCore(profileId: UniqueMechanicProfileId | string): CampaignCoreId | null {
  ...
  return getUniqueMechanicProfileMetadata(profileId)?.core ?? null;
}
...
function isKnownProfile(profileId: unknown): profileId is UniqueMechanicProfileId {
  return isCatalogProfileId(profileId);
}

function getProfileOptions(core: CampaignCoreId) {
  return getCatalogProfileOptions(core);
}
...
async function syncArkiusKineticAuraTemplate(
  ...
  const state = normalizeArkiusState(asRecord(UniqueMechanicsKernel.getState(actor).profiles)[ARKIUS_JACKER_PROFILE_ID]);
...
function findPippingSaveOrigin(message: ChatMessage): Actor | null {
  ...
  return Array.from(game.actors ?? []).find(actor =>
    ...
    && UniqueMechanicsKernel.getState(actor).activeProfile === PIPPING_PROFILE_ID
...
export class UniqueMechanicsKernel {
  ...
  static async usePippingAction(actor?: Actor | null, actionId = ""): Promise<void> {
    ...
    const reservation = await serializeExecutionMutation(target.uuid, async () => {
      const current = this.getPippingState(target);
      const effectiveTier = Math.max(current.tier, pippingTierForLevel(actorLevel)) as PippingTier;
      const availablePulse = Math.max(
        0,
        current.pulse - reservedResourceTotal(current.executions, "pulse"),
      );
      const availability = getPippingActionAvailability(
        action,
        { ...current, pulse: availablePulse },
        actorLevel,
        effectiveTier,
      );
      if (!availability.usable) {
        reservationError = availability.reason;
        return null;
      }
      const execution = createUniqueExecution({
        profileId: PIPPING_PROFILE_ID,
        actionId: action.id,
        sourceActorUuid: target.uuid,
        requesterUserId: game.user?.id,
        reservedResources: { pulse: action.pulseCost },
      });
      const executions = upsertUniqueExecution(current.executions, execution);
      const next = await this.updatePippingState(target, {
        executions,
        pendingAction: {
          actionId: action.id,
      ...
      return { execution, state: next, effectiveTier };
    });
    if (!reservation) {
      const availability = { reason: reservationError };
    ...
    const { execution: transaction, state, effectiveTier } = reservation;
    ...
    await serializeExecutionMutation(target.uuid, async () => {
      const current = this.getPippingState(target);
      await this.updatePippingState(target, {
        executions: transitionUniqueExecution(current.executions, transaction.id, "executing"),
      });
    });
    let execution;
    ...
    try {
      execution = await executePippingAction({
        ...
        reserveAdditionalPulse: async amount => serializeExecutionMutation(target.uuid, async () => {
          const current = this.getPippingState(target);
          const active = current.executions.find(entry => entry.id === transaction.id);
          if (!active || isTerminalExecutionStage(active.stage)) return false;
          const currentReservation = Number(active.reservedResources?.pulse ?? action.pulseCost);
          const otherReservations = reservedResourceTotal(current.executions, "pulse", transaction.id);
          if (current.pulse - otherReservations < currentReservation + amount) return false;
          await this.updatePippingState(target, {
            executions: upsertUniqueExecution(current.executions, {
              ...active,
              updatedAt: Date.now(),
              reservedResources: {
                ...active.reservedResources,
                pulse: currentReservation + amount,
              },
            }),
          });
          return true;
    ...
    if (!execution.completed) {
      await serializeExecutionMutation(target.uuid, async () => {
        const current = this.getPippingState(target);
        const executions = transitionUniqueExecution(
          current.executions,
          transaction.id,
          executionError ? "failed" : "cancelled",
          executionError ? { error: executionError } : {},
        );
        const latestActive = [...executions]
          .reverse()
          .find(entry => !isTerminalExecutionStage(entry.stage));
        await this.updatePippingState(target, {
          executions,
          pendingAction: latestActive
            ? {
              actionId: latestActive.actionId,
              pulseCost: Number(latestActive.reservedResources?.pulse ?? 0),
              startedAt: latestActive.createdAt,
              userId: latestActive.requesterUserId,
            }
            : undefined,
        });
      });
    ...
    await serializeExecutionMutation(target.uuid, async () => {
      const current = this.getPippingState(target);
      const totalPulseCost = action.pulseCost + (execution.additionalPulseCost ?? 0);
      const executions = transitionUniqueExecution(
        current.executions,
        transaction.id,
        "executed",
        { result: { totalPulseCost, completed: true } },
      );
      const latestActive = [...executions]
        .reverse()
        .find(entry => !isTerminalExecutionStage(entry.stage));
      const nextPatch: PartialPippingNightState = {
        pulse: current.pulse - totalPulseCost,
        executions,
        pendingAction: latestActive
          ? {
            actionId: latestActive.actionId,
      ...
      if (execution.manifestations) nextPatch.shadowManifestations = execution.manifestations;
      if (execution.animatedShadow) nextPatch.animatedShadow = execution.animatedShadow;
      if (execution.persistentArea) {
        nextPatch.persistentAreas = [
          ...current.persistentAreas.filter(area => area.actionId !== execution.persistentArea?.actionId),
