...
async function syncArkiusKineticAuraTemplate(
  ...
  const state = normalizeArkiusState(asRecord(UniqueMechanicsLegacy.getState(actor).profiles)[ARKIUS_JACKER_PROFILE_ID]);
...
function findPippingSaveOrigin(message: ChatMessage): Actor | null {
  ...
  return Array.from(game.actors ?? []).find(actor =>
    ...
    && UniqueMechanicsLegacy.getState(actor).activeProfile === PIPPING_PROFILE_ID
...
export class UniqueMechanicsLegacy {
